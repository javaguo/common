...
public class PlatformTreeUtils {
     ...
     */
	public static String  createExtTreeList(List<SysEnTreeNode> treeNodeList,boolean hasCheckedBox) throws PlatformException {
		/**
		...
		for (SysEnTreeNode tempTreeNode : treeNodeList) {
			...
			if( StringUtils.isBlank( fatherId ) || "null".equals( fatherId ) ){
			...
			for( int i=0;i<fieldsMap.length;i++ ){
				...
				try {
					...
					if( ( "id".equals(treeNodeFieldName) || "text".equals(treeNodeFieldName)  )
						...
						throw new PlatformException("树节点查询结果错误！");
					}
